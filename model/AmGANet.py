...
from .SSD import SAU, MRU
from my_open_clip.src.open_clip import create_model_from_pretrained, get_tokenizer
from .IACMA import CustomCLIP




class BERTModel(nn.Module):
    def __init__(self, take_name, project_dim):
        super().__init__()
        ...
        self.prompt = CustomCLIP(1, take_name,self.model, self.tokenizer, self.device)

        self.num_decoder_tokens = self.prompt.max_location_tokens + 1

        self.text_head = nn.Sequential(
            nn.Conv1d(self.num_decoder_tokens, 24, kernel_size=1, stride=1),
            nn.ReLU(),
            nn.Dropout(0.0)
        )
        ...
        keywords_to_update = [
            "prompt_learner_loc.ctx",
            "prompt_learner_num.ctx",
            "image_adapter",
            "text_adapter",
        ]
        ...
        for name, param in self.prompt.named_parameters():
            ...
            if any(key in name for key in keywords_to_update):
        ...
        print(f"\n✅ Current statistics on the quantity of thawing parameters:")
        print(f" - Total activation layer count / parameter block: {len(enabled)}")
        sample_active = sorted(list(enabled))[:5]
        print(f" - Activation layer example: {sample_active}")
    ...
    def forward(self, inputs):
        ...
        return:
            {
                "encoder_text": (B, project_dim),
                "decoder_text": (B, 24, project_dim),
                "location_mask": (B, K),
                "num_pred": (B,),
                "num_expected": (B,)
            }
        ...
        results = self.prompt(inputs)

        # -------------------------
        #  encoder 
        # -------------------------

        loc_global = results["location"]["pooled_feature"]   # (B, 1, 512)
        num_token = results["num"]["features"]               # (B, 1, 512)

        encoder_tokens=loc_global

        encoder_text = encoder_tokens.mean(dim=1)                    # (B, project_dim)

        # -------------------------
        #  decoder 
        # -------------------------
        loc_tokens = results["location"]["features"]                 # (B, K, 512), K=max_location_tokens

        decoder_tokens = torch.cat([loc_tokens, num_token], dim=1)   # (B, K+1, 512)

  
        decoder_text = self.text_head(decoder_tokens)               # (B, 24, project_dim)

        return {
            "encoder_text": encoder_text,                           # encoder
            "decoder_text": decoder_text,                           # decoder
            "location_mask": results["location"]["feature_mask"],   # (B, K)
            "num_pred": results["num"]["pred_count"],               # (B,)
            "num_expected": results["num"]["expected_count"],       # (B,)
            "location_indices": results["location"]["topk_indices"] # (B, K)
        }
...
class TransformerStageDecoder(nn.Module):
    def __init__(self, fmap_size, window_size, ns_per_pt,
        ...
        for i in range(depths):
            if stage_spec[i] == 'D':
                ...
                self.attns.append(MRU(dim_embed,1,fmap_size[0],1,dim_embed,dim_embed//2))
            elif stage_spec[i] == 'N':
                self.attns.append(SAU(dim_embed,1,fmap_size[0],1,dim_embed,dim_embed//2))
    ...
    def forward(self, x,text):
        ...
        for d in range(self.depths):
            ...
            if self.stage_spec[d] == 'N':
                # x0 = x
                # x, text = self.attns[d](x,text)#
                # x = self.drop_path[d](x) + x0
                x0 = x
                x, text = self.attns[d](self.layer_norms[2 * d](x),text)#消融baseline
                x = self.layer_scales[2 * d](x)
                x = self.drop_path[d](x) + x0
            else:

                x0 = x
                x, text = self.attns[d](self.layer_norms[2 * d](x),text)#
                x = self.layer_scales[2 * d](x)
                x = self.drop_path[d](x) + x0
                x0 = x
    ...
    def forward(self, x,text):
        ...
        for d in range(self.depths):
            ...
            if self.stage_spec[d] == 'X':
                ...
                x0 = x
                x, pos, ref = self.attns[d](self.layer_norms[2 * d](x),text)#
                x = self.layer_scales[2 * d](x)
                x = self.drop_path[d](x) + x0
                x0 = x
                x = self.mlps[d](self.layer_norms[2 * d + 1](x))
...
class AmGANet(nn.Module):
    def __init__(self, img_size=224, patch_size=4, expansion=4, task_name="Covid19_X", num_classes=9,
                 dim_stem=96, dims=[96, 192, 384, 768],
                 depths_Encoder=[2, 2, 6, 2], depths_Decoder=[2, 2, 6, 2],
                 heads=[3, 6, 12, 24], heads_q=[6, 12, 24, 48],
                 window_sizes=[7, 7, 7, 7], image_H=[56, 28, 14, 7],
                 drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.0,
                 strides=[-1, -1, -1, -1],
                 offset_range_factor=[1, 2, 3, 4],
                 stage_spec=[['L', 'D'], ['L', 'D'], ['L', 'D', 'L', 'D', 'L', 'D'], ['L', 'D']],
                 stage_specEN=[['L', 'D'], ['L', 'D'], ['L', 'D', 'L', 'D', 'L', 'D'], ['L', 'D']], 
                 groups=[-1, -1, 3, 6],
        ...
        #  encoder_pos_layers: [-1]  False
        #     decoder_pos_layers: [1, 2, 3]
        for i in range(len(depths_Encoder)):  # len(depths_Encoder) =4
            ...
            self.stages.append(
                TransformerStage(
                    ...
                    stage_specEN[i], groups[i], use_pes[i],
            ...
            if i < 3:
                img_size = img_size // 2  # img_size = 56,28,14,7,,,,,i = 0,1,2
        ...
        for i in range(len(depths_Decoder)):
            idx = len(depths_Decoder) - 1 - i
            self.text_project.append(nn.Sequential(
            nn.Linear(dims[idx],dims[idx]//2),
            nn.LeakyReLU(),
            ))
        ...
        self.text_encoder = BERTModel(task_name,512)
        self.text_de = nn.Sequential(             
            nn.Linear(512, 256),
            nn.LayerNorm(256),             
            nn.ReLU(),             
        )
    ...
    def forward_encoder(self, x,text):
        x = self.patch_proj(x)
        # print(x.shape)
        x_downsample = []
    ...
    def forward_decoder(self, x, x_downsample,text):
        seg_outputs = []
        for i in range(len(self.stages)):
            if i == 0:
                # print(x.shape)
                ...
                x, text = self.stages_up[i - 1](x,text)
        ...
        if len(seg_outputs) > 0:
            x = x + F.interpolate(F.interpolate(seg_outputs[-2], scale_factor=2, mode='bilinear') + seg_outputs[-1],
                               scale_factor=2, mode='bilinear')
        return x

    def forward(self, x):

        text_out = self.text_encoder(x)

        encoder_text = text_out["encoder_text"]   # (B, project_dim)
        decoder_text = text_out["decoder_text"]   # (B, 24, project_dim)
        decoder_text = self.text_de(decoder_text)

        x, x_downsample = self.forward_encoder(x,encoder_text.squeeze(1))
        x = self.forward_decoder(x, x_downsample,decoder_text)
        return x

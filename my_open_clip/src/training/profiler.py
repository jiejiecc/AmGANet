...
def profile_model(model_name, batch_size=1, profiler='torch'):
    ...
    model = my_open_clip.create_model(model_name, force_custom_text=True, pretrained_hf=False)
    model.eval()
    ...
    model_cfg = my_open_clip.get_model_config(model_name)
    if model_cfg:
        vision_cfg = my_open_clip.CLIPVisionCfg(**model_cfg['vision_cfg'])
        text_cfg = my_open_clip.CLIPTextCfg(**model_cfg['text_cfg'])
...
def main():
    ...
    if args.model == 'all':
        parsed_model = my_open_clip.list_models()
    else:

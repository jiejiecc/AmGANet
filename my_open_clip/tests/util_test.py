...
def inference_text(model, model_name, batches):
    y = []
    tokenizer = my_open_clip.get_tokenizer(model_name)
    with torch.no_grad():
        for x in batches:
            x = tokenizer(x)
...
def random_text_batch(batch_size, min_length = 75, max_length = 75):
    t = my_open_clip.tokenizer.SimpleTokenizer()
...
def create_test_data_for_model(
    ...
    if not overwrite and text_exists and image_exists:
        return
    seed_all()
    model, _, preprocess_val = my_open_clip.create_model_and_transforms(
            model_name,
...
def create_test_data(
    ...
    }).intersection(my_open_clip.list_models()))
    models.sort()
...
class TestWrapper(torch.nn.Module):
    ...
    def __init__(self, model, model_name, output_dict=True) -> None:
        ...
        if type(model) in [my_open_clip.CLIP, my_open_clip.CustomTextCLIP]:
            self.model.output_dict = self.output_dict
        config = my_open_clip.get_model_config(model_name)
...
def main(args):
    ...
    if args.git_revision is not None:
        ...
        try:
            ...
            raise e
    my_open_clip = importlib.import_module('open_clip')
    models = my_open_clip.list_models() if args.all else args.model + model_list
    try:
